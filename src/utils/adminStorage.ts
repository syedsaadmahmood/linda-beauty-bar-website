...
export async function saveTestimonial(testimonial: Testimonial): Promise<Testimonial> {
  try {
    // For new testimonials (id === 0), omit the id field so Supabase auto-generates it
    // For existing testimonials, include the id for upsert
    const row: any = {
      name: testimonial.name,
      service: testimonial.service,
      rating: testimonial.rating,
    ...
    // Only include id if it's a valid existing testimonial (id > 0)
    if (testimonial.id && testimonial.id > 0) {
      row.id = testimonial.id;
    }
    ...
    if (testimonial.id && testimonial.id > 0) {
      ...
      // Insert new testimonial (id will be auto-generated by SERIAL)
...
export async function savePortfolioItem(item: Portfolio): Promise<Portfolio> {
  try {
    // For new items (id === 0), omit the id field so Supabase auto-generates it
    // For existing items, include the id for upsert
    const row: any = {
    ...
    // Only include id if it's a valid existing item (id > 0)
    ...
    if (item.id && item.id > 0) {
      ...
      // Insert new item (id will be auto-generated by SERIAL)
      const { data: insertData, error: insertError } = await supabase
        .from(TABLES.portfolio)
        .insert(row)
        .select()
